...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        k = (
            self.key(x).view(B, T, self.G.n_head, G // self.G.n_head).transpose(1, 2)
        )  # (B, nh, T, hs)
        q = (
            self.query(x).view(B, T, self.G.n_head, G // self.G.n_head).transpose(1, 2)
        )  # (B, nh, T, hs)
        v = (
            self.value(x).view(B, T, self.G.n_head, G // self.G.n_head).transpose(1, 2)
        )  # (B, nh, T, hs)
...
class GPT(nn.Module):
    ...
    def append_location(self, x):
        ...
        return torch.cat(
            [x, X[None, ..., None].repeat_interleave(x.shape[0], 0).to(x.device)], -1
        )
    ...
    def loss(self, batch):
        ...
        lcd_loss = -dist.log_prob(
            batch['lcd'].reshape(dist.logits.shape)
        ).mean() / np.log(
            2
        )  # bits/dim
        return lcd_loss
