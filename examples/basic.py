...
def func(env):
    ...
    p, l = pl[:,:env.viewer.width-2], pl[:,env.viewer.width:]
    return p,l

pls = [func(env) for env in [envs.Dropbox(), envs.Bounce(), envs.Urchin(), envs.UrchinBall(), envs.UrchinBalls(), envs.UrchinCubes()]]
p,l = zip(*pls)
...
plt.imsave('dropbox.png', linecat([linecat(p,1), linecat(l,1)]))
