...
sortdict = lambda x: subdict(x, sorted(x))
def subdlist(dict, keys): return [dict[key] for key in keys]
get_angle = lambda sin, cos: np.arctan2(sin, cos)
# map from -1,1 to bounds
mapto = lambda a, lowhigh: ((a + 1.0) / (2.0) * (lowhigh[1] - lowhigh[0])) + lowhigh[0]
# map from bounds to -1,1
rmapto = lambda a, lowhigh: ((a - lowhigh[0]) / (lowhigh[1]-lowhigh[0]) * (2)) + -1
umapto = lambda a, from_lh, to_lh: ((a - from_lh[0]) / (from_lh[1]-from_lh[0]) * (to_lh[0]+to_lh[1])) + to_lh[0]

tileN = lambda x, N: torch.tile(x[None], [N]+[1]*len(x.shape))
