...
class RLAlgo:
    def __init__(self, G):
        ...
        if G.lenv:
            ...
            self.tvenv = self.learned_tvenv = wrappers.RewardLenv(
                wrappers.LearnedEnv(TN, model, G)
            )
            ...
            if G.preproc:
                ...
                self.tvenv = self.learned_tvenv = wrappers.PreprocVecEnv(
    ...
    def test_agent(self, itr, use_lenv=False):
        ...
        if len(frames) != 0:
            ...
            frames = frames.transpose(0, 2, 1, 3, 4).reshape(
                [-1, self.G.lcd_h * 1 * REP, TN * self.G.lcd_w * REP, 3]
            )
            # make borders
            for k in range(TN):
                if use_lenv:
            ...
            for i in range(len(frames)):
                ...
                for j in range(TN):
                    if use_lenv:
                        color = (
                            yellow
                            if dones[i][j].detach().cpu().numpy()
                            and i != self.G.ep_len - 1
                            else white
                        )
                        draw.text(
                            (self.G.lcd_w * REP * j + 10, 10),
                            f't: {i} r:{rs[i][j].detach().cpu().numpy():.3f}\nV: {vs[i][j].detach().cpu().numpy():.3f}',
                            fill=color,
                            fnt=fnt,
                        )
                        draw.text(
                            (self.G.lcd_w * REP * j + 5, 5),
                            f'{"*"*int(success[j].detach().cpu().numpy())}',
                            fill=yellow,
                            fnt=fnt,
                        )
                        ...
                        color = (
                            yellow if dones[i][j] and i != self.G.ep_len - 1 else white
                        )
                        draw.text(
                            (self.G.lcd_w * REP * j + 10, 10),
                            f't: {i} r:{rs[i][j]:.3f}\nV: {vs[i][j]:.3f}',
            ...
            utils.add_video(
                self.writer, f'{prefix}_rollout', vid, itr + 1, fps=self.G.fps
            )
