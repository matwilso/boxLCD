    ...
    def __init__(self, G, obs_space, act_space):
        ...
        for x in ['o:', 'o2:']:
            for key in obs_space.spaces:
                self.bufs[x + key] = np.zeros(
                    (size, *obs_space.spaces[key].shape), dtype=np.float32
                )
...
class ReplayBuffer:
    ...
    def store_n(self, ntrans):
        ...
        if end_idx <= self.max_size:  # normal operation
            ...
            for key in self.bufs:
                self.bufs[key][self.ptr : self.ptr + top_off] = self.proc(
                    ntrans[key][:top_off]
                )  # top off the last end of the array
                self.bufs[key][:overflow] = self.proc(
                    ntrans[key][top_off:]
