...
class SAC(RLAlgo):
  ...
  def compute_loss_q(self, data):
    q_info = {}
    alpha = self.G.alpha if not self.G.learned_alpha else th.exp(self.ac.log_alpha).detach()
  ...
  def compute_loss_pi(self, data):
    ...
    if self.G.learned_alpha:
      loss_alpha = (-1.0 * (th.exp(self.ac.log_alpha) * (logp_pi + self.ac.target_entropy).detach())).mean()
  ...
  def update(self, data):
    ...
    loss_q.backward()
    self.logger['Q/grad_norm'] += [utils.compute_grad_norm(self.ac.q1.parameters()).detach().cpu()]
    ...
    loss_pi.backward()
    self.logger['Pi/grad_norm'] += [utils.compute_grad_norm(self.ac.pi.parameters()).detach().cpu()]
  ...
  def run(self):
    ...
    for itr in range(1, self.G.total_steps+1):
      ...
      time_to_succ = pf.minimum(time_to_succ, self.G.ep_len * ~success + ep_len * success)
      # print(time_to_succ)
      ...
      if len(dixs) == self.G.num_envs or (not self.G.lenv and self.G.succ_reset):
        ...
        if len(dixs) != 0:
          ...
          if self.G.lenv:
            ...
            assert self.env.shared_memory, "i am not sure if this works when you don't do shared memory. it would need to be tested. something like the comment below"
