...
class VideoModel(Net):
    ...
    def _unprompted_eval(
        ...
        if arbiter is not None:
            ...
            if arbiter.original_name == 'TracedArbiter':
                ...
                metrics['eval/unprompted_action_log_mse'] = (
                    ((sact - paa) ** 2).mean().log()
                )
    ...
    def _duplicate_eval(self, epoch, writer, metrics, batch, arbiter=None):
        ...
        sample = self.sample(
            n, action=batch['action'], prompts=batch, prompt_n=self.G.prompt_n
        )
        if 'lcd' in sample:
            pred_lcd = sample['lcd']
    ...
    def _prompted_eval(
        ...
        if 'lcd' in sample:
            ...
            if make_video:
                ...
                self._lcd_video(
                    epoch, writer, pred_lcd, true_lcd, prompt_n=self.G.prompt_n
                )
        ...
        if 'proprio' in sample:
            ...
            metrics['eval/proprio_log_mse'] = (
                (
                    (
                        true_proprio[:, self.G.prompt_n :]
                        - pred_proprio[:, self.G.prompt_n :]
                    )
                    ** 2
                )
                .mean()
            ...
            if make_video:
                # visualize reconstruction
                self._proprio_video(
                    epoch, writer, pred_proprio, true_proprio, prompt_n=self.G.prompt_n
                )
