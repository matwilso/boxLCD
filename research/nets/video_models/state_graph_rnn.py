...
class StateGraphRNN(VideoModel):
  def __init__(self, env, G):
    ...
    self.tf1 = TransformerBlock(self.n_objects, G, causal=False)
    #self.tf2 = TransformerBlock(self.n_objects, G, causal=False)
  ...
  def forward(self, state, hc=None):
    ...
    x = self.embed(objs).flatten(0,1)
    #x = self.tf1(x)
    x = self.tf1(x).unflatten(0, [bs,t])
    x = flat(x)
