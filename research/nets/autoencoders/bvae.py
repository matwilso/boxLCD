...
class BVAE(SingleStepAE):
    def __init__(self, env, G):
        ...
        assert (
            self.G.entropy_bonus != 0.0
        )  # you should set entropy bonus if you're going to use this net
    ...
    def encode(self, batch, noise, flatten=True):
        ...
        if flatten:
            z_q = z_q.flatten(-3)
            assert (
                z_q.shape[-1] == self.z_size
            ), 'encode shape should equal the z_size. probably forgot to change one.'
        if len(shape) == 4:
