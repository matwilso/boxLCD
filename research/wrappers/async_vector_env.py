...
class AsyncVectorEnv(VectorEnv):
  ...
  def seed(self, seeds=None):
    ...
    if self._state != AsyncState.DEFAULT:
      raise AlreadyPendingCallError('Calling `seed` while waiting for a pending call to `{0}` to complete.'.format(self._state.value), self._state.value)
  ...
  def reset_wait(self, idxs, timeout=None):
    ...
    if not self._poll(timeout):
      self._state = AsyncState.DEFAULT
      raise mp.TimeoutError('The call to `reset_wait` has timed out after {0} second{1}.'.format(timeout, 's' if timeout > 1 else ''))
  ...
  def step_async(self, actions):
    ...
    self._assert_is_running()
    if self._state != AsyncState.DEFAULT:
      raise AlreadyPendingCallError('Calling `step_async` while waiting for a pending call to `{0}` to complete.'.format( self._state.value), self._state.value)
  ...
  def step_wait(self, timeout=None):
    ...
    if not self._poll(timeout):
      self._state = AsyncState.DEFAULT
      raise mp.TimeoutError('The call to `step_wait` has timed out after {0} second{1}.'.format(timeout, 's' if timeout > 1 else ''))
  ...
  def close_extras(self, timeout=None, terminate=False):
    ...
    try:
      if self._state != AsyncState.DEFAULT:
        logger.warn('Calling `close` while waiting for a pending call to `{0}` to complete.'.format(self._state.value))
