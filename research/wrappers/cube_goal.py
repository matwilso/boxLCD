...
class CubeGoalEnv:
    ...
    def observation_space(self):
        ...
        base_space.spaces['goal:full_state'] = copy.deepcopy(
            base_space.spaces['full_state']
        )
        return base_space
    ...
    def comp_rew_done(self, obs, info={}):
        ...
        delta = np.abs(
            obs['goal:full_state'][..., self.idxs] - obs['full_state'][..., self.idxs]
        ).mean()
        if self.G.diff_delt:
            last_delta = np.abs(
                obs['goal:full_state'][..., self.idxs]
                - self.last_obs['full_state'][..., self.idxs]
            ).mean()
