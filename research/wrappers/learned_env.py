...
class LearnedEnv:
    def __init__(self, num_envs, model, G):
        ...
        self.action_space = gym.spaces.Box(
            -1, +1, (num_envs,) + model.action_space.shape, model.action_space.dtype
        )
        self.model.eval()
        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                ...
                self.window_batch = self.model.onestep(
                    self.window_batch, self.ptr, temp=self.G.lenv_temp
                )
                obs = {
                    key: val[:, self.ptr]
    ...
    def _reset_goals(self, mask, logger=defaultdict(lambda: [])):
        with torch.no_grad():
            ...
            if self.G.lenv_goals:
                ...
                new_goal = utils.prefix_dict(
                    'goal:', self.lenv.reset(update_window_batch=False)
                )
                new_goal = utils.prefix_dict(
                    'goal:',
                    utils.filtdict(
                        self.lenv.reset(update_window_batch=False),
                        '(lcd|proprio|object)',
                    ),
                )
            else:
    ...
    def comp_rew_done(self, obs, info={}):
        ...
        if 'BodyGoal' in self.real_env.__class__.__name__:
            ...
            if self.obj_loc is None:
                ...
                goal = self.obj_loc(
                    utils.filtdict(obs, 'goal:', fkey=lambda x: x[5:])
                ).detach()
