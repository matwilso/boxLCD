...
for env in envs:
    ...
    for tag in tags:
        ...
        plt.plot(realdf.Step, smooth(realdf.Value, weight=smooth_map[tag]), linewidth=2.0, color='C0', label=mode_map['real']+mmap[f'{env}real{tag}'])
        ...
        plt.plot(lenvdf.Step, smooth(lenvdf.Value, weight=smooth_map[tag]), linewidth=2.0, color='C1', label=mode_map['lenv']+mmap[f'{env}lenv{tag}'])
