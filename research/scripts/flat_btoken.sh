...
for env in ${envs[@]}; do
    ...
    python -m research.main --mode=train --env=$env --datapath=logs/datadump/$env/ --arbiterdir=logs/arbiter/$env --model=FlatBToken --lr=5e-4 --log_n=1000 --bs=32 --n_layer=2 --n_head=4 --n_embed=256 --hidden_size=256 --window=$window --total_itr=100000 --prompt_n=$prompt_n --logdir=logs/video/FlatBToken/$env/ --weightdir=logs/autoencoder/BVAE/$env/
