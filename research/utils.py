...
def subdict(dict, subkeys):
    return {key: dict[key] for key in subkeys}


def sortdict(x):
    return subdict(x, sorted(x))


def subdlist(dict, subkeys):
    return [dict[key] for key in subkeys]


# filter or negative filter
def filtdict(dict, phrase, fkey=lambda x: x, fval=lambda x: x):
    return {
        fkey(key): fval(dict[key]) for key in dict if re.match(phrase, key) is not None
    }


def nfiltdict(dict, phrase):
    return {key: dict[key] for key in dict if re.match(phrase, key) is None}


def filtlist(list, phrase):
    return [item for item in list if re.match(phrase, item) is not None]


def nfiltlist(list, phrase):
    return [item for item in list if re.match(phrase, item) is None]
...
def mapto(a, lowhigh):
    return ((a + 1.0) / (2.0) * (lowhigh[1] - lowhigh[0])) + lowhigh[0]


# map from bounds to -1,1
def rmapto(a, lowhigh):
    return ((a - lowhigh[0]) / (lowhigh[1] - lowhigh[0]) * (2)) + -1
...
def force_shape(out):
    ...
    if isinstance(out, np.ndarray):
        ...
        out = np.concatenate(
            [out, np.zeros(out.shape[:-1], dtype=out.dtype)[..., None]], -1
        )
    else:
...
def combine_rgbs(arr, row=5, col=5):
    ...
    if len(arr.shape) == 4:  # image
        ...
        x = permute(arr.reshape([row, col, C, H, W]), (2, 0, 3, 1, 4)).reshape(
            [C, row * H, col * W]
        )
        return x
        ...
        x = permute(arr.reshape([row, col, T, C, H, W]), (2, 3, 0, 4, 1, 5)).reshape(
            [T, C, row * H, col * W]
        )
        return x
    else:
...
def combine_imgs(arr, row=5, col=5):
    ...
    if len(arr.shape) == 4:  # image
        ...
        if isinstance(arr, np.ndarray):
            x = (
                arr.reshape([row, col, T, H, W])
                .transpose(2, 0, 3, 1, 4)
                .reshape([T, row * H, col * W])
            )
        else:
            x = (
                arr.reshape([row, col, T, H, W])
