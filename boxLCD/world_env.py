...
from boxLCD.world_defs import ROBOT_FILLER, SCALE
...
class WorldEnv(gym.Env, EzPickle):
    ...
    ENV_DG = utils.AttrDict()
    ENV_DG.base_dim = 5  # base size of box2D physics world
    ENV_DG.lcd_base = 16  # base size of lcd rendered image. this represents the height. width = wh_ratio*height
    ENV_DG.wh_ratio = 2.0  # width:height ratio of the world and images
    ENV_DG.ep_len = 100  # length to run episode before done timeout
    ...
    def __init__(self, world_def, G={}):
        """
        args:
          world_def: description of what components you want in the world.
          G: things you might set as cmd line arguments
        """
        EzPickle.__init__(self)
        # env definitions
        self.world_def = world_def
        # CONFIG
        self.G = utils.AttrDict(self.ENV_DG)
        ...
        for i in range(len(self.world_def.robots)):
            ...
            self.world_def.robots[i] = robot = ROBOT_FILLER[robot.type](robot, self.G)
        ...
        if (
            len(self.world_def.robots) == 0
        ):  # because having a zero shaped array makes things break
        ...
        if self.pobs_size == 0:
            ...
            spaces['proprio'] = gym.spaces.Box(
                -1, +1, (self.pobs_size,), dtype=np.float32
            )
        spaces['lcd'] = gym.spaces.Box(
            0,
            1,
            (self.G.lcd_base, int(self.G.lcd_base * self.G.wh_ratio), 3),
            dtype=np.uint8,
        )
    ...
    @property
    def WIDTH(self):
        return int(self.G.wh_ratio * self.G.base_dim)

    @property
    def HEIGHT(self):
        return self.G.base_dim
    ...
    def _reset_bodies(self):
        ...
        for robot in self.world_def.robots:
            ...
            robot_color = to_rgb(robot.color)
            fixture = fixtureDef(
                shape=root_body.shape,
                density=1.0 if root_body.density is None else root_body.density,
                categoryBits=root_body.categoryBits,
                maskBits=root_body.maskBits,
                friction=1.0,
            )
            ...
            root_xy = A[
                self._sample(name + ':x:p', -rangex, rangex),
                self._sample(name + ':y:p', -rangey, -rangey),
            ]
            root_angle = np.arctan2(
                self._sample(name + ':sin'), self._sample(name + ':cos')
            )
        ...
        # OBJECT
        for obj in self.world_def.objects:
            if np.random.binomial(1, obj.dropout):
                continue

            # MAKE SHAPE AND SIZE
            obj_size = obj.size
            obj_shapes = {
                'circle': circleShape(radius=obj_size, pos=(0, 0)),
                'box': (polygonShape(box=(obj_size, obj_size))),
            }
            shape_name = (
                list(obj_shapes.keys())[np.random.randint(len(obj_shapes))]
                if obj.shape == 'random'
                else obj.shape
            )
            ...
            # if no robots, drop cubes from up high for more interestingness
            if len(self.world_def.robots) == 0:
                pos = A[
                    (
                        self._sample(obj.name + ':x:p', -rangex, rangex),
                        self._sample(obj.name + ':y:p', -rangey, rangey),
                    )
                ]
            else:
                pos = A[
                    (
                        self._sample(obj.name + ':x:p', -rangex, rangex),
                        self._sample(obj.name + ':y:p', -rangey, -0.25),
                    )
                ]
            # ANGLE OR NOT
            if obj.rand_angle:
                angle = np.arctan2(
                    self._sample(obj.name + ':sin'), self._sample(obj.name + ':cos')
                )
            else:
                angle = 0
            # CREATE OBJ BODY
    ...
    def reset(self, full_state=None, proprio=None):
        ...
        if self.G.walls:
            X = 0.0
            self.statics['wall1'] = self.b2_world.CreateStaticBody(
                shapes=edgeShape(vertices=[(0, 0), (self.WIDTH + X, 0)])
            )
            self.statics['wall2'] = self.b2_world.CreateStaticBody(
                shapes=edgeShape(vertices=[(0, 0), (0, self.HEIGHT + X)])
            )
            self.statics['wall3'] = self.b2_world.CreateStaticBody(
                shapes=edgeShape(
                    vertices=[(self.WIDTH + X, 0), (self.WIDTH + X, self.HEIGHT)]
                )
            )
            self.statics['wall4'] = self.b2_world.CreateStaticBody(
                shapes=edgeShape(
                    vertices=[(0, self.HEIGHT + X), (self.WIDTH + X, self.HEIGHT + X)]
                )
            )
        else:
            self.statics['floor'] = self.b2_world.CreateStaticBody(
                shapes=edgeShape(
                    vertices=[(-1000 * self.WIDTH, 0), (1000 * self.WIDTH, 0)]
                )
            )
        self._reset_bodies()
        self.current_background_color = to_rgb(self.world_def.background_color)

        if proprio is not None:
            assert (
                proprio.shape[-1] == self.observation_space.spaces['proprio'].shape[-1]
            ), f'invalid shape for proprio {proprio.shape} {self.observation_space.spaces["proprio"]}'
            full_state = np.zeros(self.observation_space.spaces['full_state'].shape)
            full_state[self.pobs_idxs] = proprio
        if full_state is not None:
            full_state = utils.NamedArray(
                np.array(full_state).astype(np.float), self.obs_info
            )
            ...
            for obj in self.world_def.objects:
                ...
                self.dynbodies[name].position = xy = full_state[
                    f'{name}:x:p', f'{name}:y:p'
                ]
                self.dynbodies[name].angle = np.arctan2(
                    full_state(name + ':sin'), full_state(name + ':cos')
                )

            for robot in self.world_def.robots:
                name = robot.name + ':root'
                self.dynbodies[f'{name}'].position = root_xy = full_state[
                    f'{name}:x:p', f'{name}:y:p'
                ]
                self.dynbodies[f'{name}'].angle = root_angle = np.arctan2(
                    full_state(name + ':sin'), full_state(name + ':cos')
                )
                parent_angles = {}
                parent_angles[name] = root_angle
                ...
                for bj_name in robot.joints:
                    ...
                    ab_delta = rot.dot(ab_delta)
                    if self.G.root_offset:
                        self.dynbodies[name].position = self.joints[
                            name
                        ].bodyB.transform.position = pos = (
                            A[root_xy]
                            + A[(full_state[name + ':x:p'], full_state[name + ':y:p'])]
                        )
                    else:
                        self.dynbodies[name].position = self.joints[
                            name
                        ].bodyB.transform.position = pos = A[
                            (full_state[name + ':x:p'], full_state[name + ':y:p'])
                        ]

                    offset_angle = np.arctan2(
                        full_state[name + ':sin'], full_state[name + ':cos']
                    )
                    if self.G.angular_offset:
        ...
        if not self.G.walls:
            self.scroll = (
                self.dynbodies[f'{self.world_def.robots[0].type}0:root'].position.x
                - self.VIEWPORT_W / SCALE / 2
            )
    ...
    def _get_obs(self):
        ...
        # GRAB OBJECT INFO
        for obj in self.world_def.objects:
            if obj.name in self.dynbodies:
                body = self.dynbodies[obj.name]
                (
                    full_state[f'{obj.name}:x:p'],
                    full_state[f'{obj.name}:y:p'],
                ) = body.position
                full_state[f'{obj.name}:cos'] = np.cos(body.angle)
                full_state[f'{obj.name}:sin'] = np.sin(body.angle)
            else:
                full_state[f'{obj.name}:x:p'], full_state[f'{obj.name}:y:p'] = (
                    np.nan,
                    np.nan,
                )
                full_state[f'{obj.name}:cos'] = np.nan
                full_state[f'{obj.name}:sin'] = np.nan

        # GRAB ROBOT INFO
        for robot in self.world_def.robots:
            root = self.dynbodies[robot.name + ':root']
            (
                full_state[f'{robot.name}:root:x:p'],
                full_state[f'{robot.name}:root:y:p'],
            ) = root_xy = root.position
            ...
            for joint_name, joint in robot.joints.items():
                ...
                if self.G.compact_obs:
                    full_state[f'{robot.name}:{joint_name}:angle'] = jnt.angle
                else:
                    if self.G.root_offset:
                        (
                            full_state[f'{robot.name}:{joint_name}:x:p'],
                            full_state[f'{robot.name}:{joint_name}:y:p'],
                        ) = (
                            jnt.bodyB.transform.position - root_xy
                        )
                    else:
                        (
                            full_state[f'{robot.name}:{joint_name}:x:p'],
                            full_state[f'{robot.name}:{joint_name}:y:p'],
                        ) = jnt.bodyB.transform.position
                    if self.G.angular_offset:
    ...
    def step(self, action):
        ...
        for robot in self.world_def.robots:
            for jname, joint in robot.joints.items():
                ...
                if self.G.use_speed:
                    self.joints[name].motorSpeed = float(
                        joint.speed * np.clip(action[name + ':speed'], -1, 1)
                    )
                else:
                    self.joints[name].motorSpeed = float(
                        joint.speed * np.sign(action[name + ':torque'])
                    )
                    self.joints[name].maxMotorTorque = float(
                        joint.torque * np.clip(np.abs(action[name + ':torque']), 0, 1)
                    )
        # RUN SIM STEP
        if self.FPS < 30:
    ...
    def _lcd_render(self, width=None, height=None):
        ...
        for name, body in self.dynbodies.items():
            ...
            if isinstance(shape, circleShape):
                ...
                draw.ellipse(
                    topleft.tolist() + botright.tolist(), fill=color, outline=outline
                )
            else:
