...
class IndexEnv(gym.Env, EzPickle):
    ...
    Instead of just vectors for obs and act, they are named. 
    """
    ...
    def pack_info(self):
        ...
        self.observation_space = spaces.Box(-1, +1, (self.obs_info['size'],), dtype=np.float32)
    ...
    def get_act_dict(self, act_vec, map=True):
        ...
        """
        act_dict = {}
        for i, key in enumerate(self.act_info['keys']):
    ...
    def get_obs_dict(self, obs_vec, map=True):
        ...
        for i, key in enumerate(self.obs_info['keys']):
            bounds = self.obs_info[key]
            try:
